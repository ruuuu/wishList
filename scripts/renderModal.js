...
export const renderModal = ({title, description, btnSubmit, submitHandler}) => {
      ...
      const modalInputPassword = createElement('input', 
            { 
                  className: 'modal__input', 
                  type: 'password',
                  name: 'password',   // для FormData
                  placeholder: 'Пароль',
                  required: true   
            });


      const modalSubmitBtn = createElement('button',  // кнопка отправки формы
            {  
      ...
      const modalCloseBtn = createElement('button',   // кнопка закрытия формы
            {  
